...
interface Task { id: number; p1: PokemonSet[]; p2: PokemonSet[]; seed: [number, number, number, number]; depth?: number; budgetMs?: number }
...
rl.on('line', line => {
  ...
  void (async () => {
    try {
      ...
      const r = await playGame(task.p1, task.p2, { seed: task.seed, policy });
