...
interface Task { id: number; mine: PokemonSet[]; oppSets: PokemonSet[]; oppAnchor: string; depth: number; budgetMs?: number; bringK?: number; oppBringK?: number }
...
rl.on('line', line => {
  ...
  try {
    const m = evaluateMatchup(task.mine, task.oppSets, task.oppAnchor, task.depth, task.budgetMs, { bringK: task.bringK, oppBringK: task.oppBringK });
