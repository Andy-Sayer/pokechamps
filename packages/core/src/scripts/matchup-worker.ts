...
interface Task { id: number; mine: PokemonSet[]; oppSets: PokemonSet[]; oppAnchor: string; depth: number }
...
rl.on('line', line => {
  ...
  try {
    const m = evaluateMatchup(task.mine, task.oppSets, task.oppAnchor, task.depth);
