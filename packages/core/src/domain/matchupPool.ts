...
export interface MatchupTask { mine: PokemonSet[]; oppSets: PokemonSet[]; oppAnchor: string; depth: number }
...
export class MatchupPool {
  ...
  async run(tasks: MatchupTask[]): Promise<Matchup[]> {
    this.ensure();
    if (!this.usable) {
      return tasks.map(t => evaluateMatchup(t.mine, t.oppSets, t.oppAnchor, t.depth));
    }
    try {
