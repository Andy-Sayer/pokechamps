...
export const BATTLE_COMMANDS: readonly BattleCommand[] = [
  ...
  { id: 'moves',    aliases: ['moves'],                description: 'Per-move damage rows in the grid: auto (fit to terminal height) → off → on' },
  ...
  { id: 'foresight', aliases: ['foresight', 'fs'],    description: 'Opponent lookahead model for the background search: /foresight N = they pick each turn\'s move seeing N plies ahead (search runs MUCH deeper; never claims a forced win), /foresight off = full maximin (exact, default). Use ≥2 when a Perish Song is in play. Sticky.' },
  ...
  { id: 'watch',    aliases: ['watch'],               description: 'Auto-read the live feed into ratify proposals. Ctrl+W = start/stop; the layout (your own screen vs a GameShare) is auto-detected. /watch full or /watch share forces one. Run "serve" first.' },
