...
**State:** Leech Seed residual (drain + heal, clears on switch-out); drain-move self-heal (Giga Drain / Drain Punch / …); move self-stat drops (Overheat / Leaf Storm / Draco Meteor / Close Combat / Hammer Arm — Contrary inverts); Spicy Spray on-hit burn (defender ability resolved through mega). **Tier 1 quick wins now shipped:** status-move auto-apply (Will-O-Wisp/T-Wave/Spore/Toxic/Glare/Nuzzle), setup self-boost moves (Swords Dance/Dragon Dance/etc.), recoil (Brave Bird/Wood Hammer/etc. + Rock Head blocks), Toxic Orb / Flame Orb EOT status, berry auto-consume thresholds (Sitrus/pinch/Lum/status berries), Regenerator on switch-out, Rough Skin / Iron Barbs on-hit chip, Liquid Ooze (reverses drain), Bad Dreams (chips sleeping foes), weather abilities (Rain Dish/Dry Skin/Ice Body/Solar Power), Salt Cure (1/8 or 1/4 for Water/Steel), partial trap (Fire Spin/Bind — counter + chip), Curse, Aqua Ring, Ingrain, Nightmare.
