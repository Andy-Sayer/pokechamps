...
| Priority | Aqua Jet, Bullet Punch, Sucker Punch, Extreme Speed | ✅ | ✅ | ✅ | A *distinct* priority-attack option per actor×foe (`PRIO_BASE` sentinel, `my/oppPrioCell`): the max-damage `off`/`thr` cell hid priority moves whenever a stronger normal move existed (Kingambit's Kowtow out-damages its Sucker Punch), so a priority KO was invisible. Now the search can pick MY priority KO and prices the opp's (`scariestIncoming` flags it + marks it as moving first). Offered at **every ply**: at the ROOT for any damaging priority move (so the recommendation sees the chip-then-partner-KO line), and at DEPTH gated to "can KO the foe at current HP" so the lookahead value foresees a priority revenge-KO (e.g. Sucker Punch punishing my setup) without exploding branching. Sucker Punch / Thunderclap fail unless the target is using a damaging attack (`targetWillAttack`). **Effective** priority via `effectiveAttackPriority` folds in the conditional bumps: **Grassy Glide** (+1 in Grassy Terrain), **Gale Wings** (+1 on a Flying move at full HP), and **Triage** (+3 on a draining move — Giga Drain / Drain Punch / Draining Kiss) are offered as priority options and the cell's stored priority is overridden to the effective value. **GAP:** terrain/HP read from the ROOT state, so a same-turn dynamic change (a switch-in sets Grassy Terrain, partner uses Grassy Glide) isn't reflected; Quick Claw (random) is deliberately excluded. Psychic-Terrain block ✅ |
| Protect family | Protect/Detect/King's Shield/Spiky Shield/… | – | ✅ | ✅ | consecutive-use fail modelled. **On-contact punish** (`protectPunish`): a CONTACT move blocked by King's Shield (−1 Atk), Spiky Shield (1/8 chip), Silk Trap (−1 Spe), Obstruct (−2 Def), Baneful Bunker (poison) or Burning Bulwark (burn) applies that secondary to the attacker (drops route through the foe-drop accumulator so Defiant/immunity apply; status respects type immunity) |
| Switch / pivot | U-turn, Volt Switch, Flip Turn, Parting Shot, Teleport, Chilly Reception | ✅(dmg) | ✅ | ✅ | voluntary switch + **pivot**: user chips/debuffs (Parting Shot −1 Atk/SpA) then auto-switches to the best bench mon (root-ply). Fast-pivot model (bench in from start; chip before the foe acts). `PIVOT_BASE` action + per-move pivot cells. **Deep-switch arc:** both sides' bench/phantom switches are now enumerated at the first `SWITCH_PLY_LIMIT` (2) DEEPER plies too, not just the root (`value()`), affordable because each opp mon is a coarse K-spread profile (`coarseSearchProfile`, K=3) and bench cells are pre-built. Step C (confidence-adaptive K / depth) still open. |
...
| Wide/Quick Guard | — | – | ✅(noted) | ✅ | `WIDE_GUARD` / `QUICK_GUARD` actions: side-wide protect for the turn. Wide Guard zeroes the foes' SPREAD moves; Quick Guard zeroes the foes' PRIORITY moves (including Fake Out). Offered at **every ply**; checked as side booleans in `resolveTurn` so order doesn't matter. Per `@pkmn/sim` (Gen 9) these do **not** have Protect's diminishing success — their `onTry` only checks `willAct()`, so 100% every turn is correct. (They `addVolatile('stall')`, so they bump a *following* Protect's fail counter — a cross-move interaction not worth modelling over a short search.) |
| Helping Hand | — | ✅(calc) | – | ✅ | `HELP_HAND` action: the ally's moves deal ×1.5 this turn (folded into `myDmg`/`oppDmg`, so single-target / spread / priority / Fake Out chip are all boosted). Offered at **every ply** when a knower has a live partner. (Offered whenever a partner exists — not gated on whether the boost enables a KO.) |
| Strength Sap | — | – | – | ✅ | `SAP` action (Sinistcha): heals the caster by the target's **Attack stat** (`actualStat`, as % of the caster's max HP) and drops the target's Attack −1 (routed through the foe-drop accumulator). **Simplification:** auto-targets the highest-Attack live foe (the dominant sap) rather than enumerating a target; heal uses the unboosted Atk stat. |
| Fake Out | — | ✅(dmg) | ✅ | ✅ | guaranteed flinch (target can't act) when the user is first-turn-out (+3 priority); `FAKEOUT` action. `firstTurnOut` is threaded from the live match (`searchInputFromMatch` via `itemSignals.firstTurnOut`) so it fires at turn 1, plus in-search switch-ins. Inner Focus / Covert Cloak suppress the flinch (chip still applies); opp-side immunity only fires when the ability/item is *known* (opp-conservatism) |
...
| Hazard CLEAR | Defog/Rapid Spin/Mortal Spin/Court Change/Tidy Up | – | ✅ | ✅ | `CLEAR_HAZARD` action: Rapid Spin/Mortal Spin clear own side, Defog/Tidy Up clear both, Court Change swaps; Rapid Spin/Tidy Up also boost the user. Offered at the root when the user's own side actually has hazards. Defog's opp-screen clear is approximate |
