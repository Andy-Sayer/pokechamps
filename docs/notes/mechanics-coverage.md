...
| Priority | Aqua Jet, Bullet Punch, Sucker Punch, Extreme Speed | ✅ | ✅ | ✅ | A *distinct* priority-attack option per actor×foe (`PRIO_BASE` sentinel, `my/oppPrioCell`): the max-damage `off`/`thr` cell hid priority moves whenever a stronger normal move existed (Kingambit's Kowtow out-damages its Sucker Punch), so a priority KO was invisible. Now the search can pick MY priority KO and prices the opp's (`scariestIncoming` flags it + marks it as moving first). Offered at **every ply** (root + deeper lookahead), gated to "can KO the foe at current HP" so branching stays sparse — the value foresees a priority revenge-KO (e.g. Sucker Punch punishing my setup). Sucker Punch / Thunderclap fail unless the target is using a damaging attack (`targetWillAttack`). **Effective** priority via `effectiveAttackPriority` folds in the conditional bumps: **Grassy Glide** (+1 in Grassy Terrain), **Gale Wings** (+1 on a Flying move at full HP), and **Triage** (+3 on a draining move — Giga Drain / Drain Punch / Draining Kiss) are offered as priority options and the cell's stored priority is overridden to the effective value. **GAP:** terrain/HP read from the ROOT state, so a same-turn dynamic change (a switch-in sets Grassy Terrain, partner uses Grassy Glide) isn't reflected; Quick Claw (random) is deliberately excluded. Psychic-Terrain block ✅ |
...
| Wide/Quick Guard | — | – | ✅(noted) | ✅ | `WIDE_GUARD` / `QUICK_GUARD` actions: side-wide protect for the turn. Wide Guard zeroes the foes' SPREAD moves; Quick Guard zeroes the foes' PRIORITY moves (including Fake Out). Offered at the root; checked as side booleans in `resolveTurn` so order doesn't matter. **GAP:** root-only; consecutive-use fail (shared protect counter) not modelled — offered every turn. |
| Helping Hand | — | ✅(calc) | – | ✅ | `HELP_HAND` action: the ally's moves deal ×1.5 this turn (folded into `myDmg`/`oppDmg`, so single-target / spread / priority / Fake Out chip are all boosted). Offered at the root when a knower has a live partner. **GAP:** root-only (not at depth); the boost-enables-a-KO synergy isn't a gating signal (offered whenever a partner exists). |
